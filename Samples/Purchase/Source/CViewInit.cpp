...
void CViewInit::onUpdate()
{
	...
	case CViewInit::DownloadBundles:
	{
		...
		listBundles.push_back("SamplePurchase.zip");
	...
	case CViewInit::InitScene:
	{
		...
		CLocalize::getInstance()->init("SamplePurchase/Configs/Text.csv");
		CLocalize::getInstance()->setLanguage(ELanguage::VN);

		CShopConfig::getInstance()->init("SamplePurchase/Configs/Shop.csv");
		CPurchase::getInstance()->init();
	...
	default:
	{
		...
		CViewManager::getInstance()->getLayer(0)->changeView<CViewShop>();
		CViewManager::getInstance()->getLayer(1)->pushView<CViewHeader>();
	}
	break;
	}
}
