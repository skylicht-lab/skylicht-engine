...
void CViewInit::initScene()
{
	...
	cubeObj->getTransformEuler()->setPosition(core::vector3df(0.0f, -2.0f, 0.0f));

	COcclusionQuery* query = cubeObj->addComponent<COcclusionQuery>();
	core::aabbox3df box;
	float scale = 0.51; // should large than cube
	box.MinEdge.set(-scale, -scale, -scale);
	box.MaxEdge.set(scale, scale, scale);
	query->setAABBox(box);

	cubeObj->addComponent<CTestMovement>();
