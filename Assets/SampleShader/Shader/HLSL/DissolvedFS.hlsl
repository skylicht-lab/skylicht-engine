...
Texture2D uTexNoiseMap : register(t1);
SamplerState uTexNoiseMapSampler : register(s1);
...
{
	float dissolve = input.color.r - uCutoff.w;
	if (dissolve <= 0.0)
		discard;
	...
	// Emission Noise
	float noiseSize = 2.0;
	float noise = step(0.5, uTexNoiseMap.Sample(uTexNoiseMapSampler, input.tex0 * noiseSize).x);
