...
float2 ssrRayMarch(const float4 position, const float3 reflection)
{
	...
	const float2 uvOffset = float2(0.5, 0.5);
	const float2 uvScale = float2(0.5, -0.5);
	{	rayPosition += dir;	projectedCoord = mul(float4(rayPosition.xyz, 1.0), uProjection);	projectedCoord.xy = projectedCoord.xy / projectedCoord.w;	ssrUV = uvScale * projectedCoord.xy + uvOffset;	testPosition = uTexPosition.Sample(uTexPositionSampler, ssrUV);	if(rayPosition.z - testPosition.w >= 0.0)	{		ssrUV = binarySearch(dir, rayPosition);		return ssrUV;	}};
	{	rayPosition += dir;	projectedCoord = mul(float4(rayPosition.xyz, 1.0), uProjection);	projectedCoord.xy = projectedCoord.xy / projectedCoord.w;	ssrUV = uvScale * projectedCoord.xy + uvOffset;	testPosition = uTexPosition.Sample(uTexPositionSampler, ssrUV);	if(rayPosition.z - testPosition.w >= 0.0)	{		ssrUV = binarySearch(dir, rayPosition);		return ssrUV;	}};
	{	rayPosition += dir;	projectedCoord = mul(float4(rayPosition.xyz, 1.0), uProjection);	projectedCoord.xy = projectedCoord.xy / projectedCoord.w;	ssrUV = uvScale * projectedCoord.xy + uvOffset;	testPosition = uTexPosition.Sample(uTexPositionSampler, ssrUV);	if(rayPosition.z - testPosition.w >= 0.0)	{		ssrUV = binarySearch(dir, rayPosition);		return ssrUV;	}};
	{	rayPosition += dir;	projectedCoord = mul(float4(rayPosition.xyz, 1.0), uProjection);	projectedCoord.xy = projectedCoord.xy / projectedCoord.w;	ssrUV = uvScale * projectedCoord.xy + uvOffset;	testPosition = uTexPosition.Sample(uTexPositionSampler, ssrUV);	if(rayPosition.z - testPosition.w >= 0.0)	{		ssrUV = binarySearch(dir, rayPosition);		return ssrUV;	}};
	{	rayPosition += dir;	projectedCoord = mul(float4(rayPosition.xyz, 1.0), uProjection);	projectedCoord.xy = projectedCoord.xy / projectedCoord.w;	ssrUV = uvScale * projectedCoord.xy + uvOffset;	testPosition = uTexPosition.Sample(uTexPositionSampler, ssrUV);	if(rayPosition.z - testPosition.w >= 0.0)	{		ssrUV = binarySearch(dir, rayPosition);		return ssrUV;	}};
