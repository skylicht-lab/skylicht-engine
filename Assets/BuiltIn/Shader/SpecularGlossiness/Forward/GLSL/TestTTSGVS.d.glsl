...
mat4 getTransformFromTexture(vec2 p)
{
	...
	vec4 c1 = textureLod(uTransformTexture, uv, 0);
	uv.x = uv.x + nextPixelX;
	vec4 c2 = textureLod(uTransformTexture, uv, 0);
	uv.x = uv.x + nextPixelX;
	vec4 c3 = textureLod(uTransformTexture, uv, 0);
	uv.x = uv.x + nextPixelX;
	vec4 c4 = textureLod(uTransformTexture, uv, 0);
	uv.x = uv.x + nextPixelX;
