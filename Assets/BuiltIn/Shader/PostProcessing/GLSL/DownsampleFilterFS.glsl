...
void main(void)
{
	...
	sum += textureLod(uSourceTex, varTexCoord0 + vec2(0.0, -3.0 * uTexelSize.y), 0.0).rgb * weights[0];
	...
	sum += textureLod(uSourceTex, varTexCoord0 + vec2(0.0, 3.0 * uTexelSize.y), 0.0).rgb * weights[4];
