...
void main(void)
{
	...
	sum += textureLod(uSourceTex, varTexCoord0 + vec2(0.0, -2.0 * uTexelSize.y), 0.0).rgb * weights[1];
	sum += textureLod(uSourceTex, varTexCoord0, 0.0).rgb * weights[2];
	sum += textureLod(uSourceTex, varTexCoord0 + vec2(0.0, 2.0 * uTexelSize.y), 0.0).rgb * weights[3];
	sum += textureLod(uSourceTex, varTexCoord0 + vec2(0.0, 4.0 * uTexelSize.y), 0.0).rgb * weights[4];
