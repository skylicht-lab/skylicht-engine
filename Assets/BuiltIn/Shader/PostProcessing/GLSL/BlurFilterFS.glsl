...
void main(void)
{
	...
	sum += textureLod(uSourceTex, varTexCoord0 + vec2(-2.0 * uTexelSize.x, 0.0), 0.0).rgb * weights[0];
	...
	sum += textureLod(uSourceTex, varTexCoord0 + vec2(2.0 * uTexelSize.x, 0.0), 0.0).rgb * weights[4];
