...
float4 main(PS_INPUT input) : SV_TARGET
{
	...
	sum += uSourceTex.SampleLevel(uSourceTexSampler, input.tex0 + float2(0.0, -1.0 * uTexelSize.y), 0).rgb * weights[0];
	sum += uSourceTex.SampleLevel(uSourceTexSampler, input.tex0, 0).rgb * weights[1];
	sum += uSourceTex.SampleLevel(uSourceTexSampler, input.tex0 + float2(0.0, 1.0 * uTexelSize.y), 0).rgb * weights[2];
