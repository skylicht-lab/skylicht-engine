...
namespace core
{
	...
	class CMatrix4
	{
		public:
			...
			//! Simple operator for directly accessing every element of the matrix.
			const T& operator()(const s32 row, const s32 col) const { return M[ (row << 2) + col]; }
	...
	//! multiply by another matrix
	template <class T>
	inline CMatrix4<T> CMatrix4<T>::operator*(const CMatrix4<T>& matrix2) const
	{
#if defined ( USE_MATRIX_TEST )
		// Testing purpose..
		if ( this->isIdentity() )
		...
		CMatrix4<T> ret ( EM4CONST_NOTHING );

		const T *m1 = M;
		const T *m2 = matrix2.M;
		T *m3 = ret.M;
	...
	inline void CMatrix4<T>::transformBoxEx(core::aabbox3d<f32>& box) const
	{
		...
		for (u32 i = 0; i < 3; ++i)
		{
			for (u32 j = 0; j < 3; ++j)
			{
				...
				const f32 a = t * Amin[j];
				const f32 b = t * Amax[j];
