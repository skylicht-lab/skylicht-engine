...
namespace video
{
	...
	*/
	...
	public:
		...
		SMaterial()
		: MaterialType(0), AmbientColor(255,255,255,255), DiffuseColor(255,255,255,255),
			...
			FogEnable(false), NormalizeNormals(false), UseMipMaps(true), MaterialInfo(NULL)
		{ }
		...
		void setFlag(E_MATERIAL_FLAG flag, bool value, int intValue = 0)
		{
			switch (flag)
			{
				...
				case EMF_ANISOTROPIC_FILTER:
				{
					if (value)
						for (u32 i=0; i<MATERIAL_MAX_TEXTURES; ++i)
							TextureLayer[i].AnisotropicFilter = intValue;
					else
						for (u32 i=0; i<MATERIAL_MAX_TEXTURES; ++i)
