...
void CImage::swapBG()
{
    if (IsCompressed)
    {
        os::Printer::log("IImage::swapBG method doesn't work with compressed images.", ELL_WARNING);
        return;
    }
...
void CImage::copyToScaling(void* target, u32 width, u32 height, ECOLOR_FORMAT format, u32 pitch, bool swapBG)
{
	if (IsCompressed)
	{
	...
	if (Format != ECF_A8R8G8B8 && Format != ECF_R8G8B8)
		swapBG = false;
