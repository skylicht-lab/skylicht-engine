...
namespace irr
{
	...
	{
		//! constructor
		CD3D11TextureArray::CD3D11TextureArray(IImage** image, u32 arraySlices, CD3D11Driver* driver, u32 flags, const io::path& name)
		...
		{
#ifdef _DEBUG
			setDebugName("CD3D11TextureArray");
#endif
			...
			if (image)
			{
				if (createTexture(flags, image) == false)
					os::Printer::log("Could not create Direct3D11 Texture.", ELL_WARNING);
			}
		}

		//! fbo
		CD3D11TextureArray::CD3D11TextureArray(const core::dimension2d<u32>& size, u32 arraySlices, CD3D11Driver* driver, const io::path& name, const ECOLOR_FORMAT format)
			:ITexture(name), Texture(0), TextureBuffer(0),
			Device(0), Context(0), Driver(driver),
			SRView(0),
			TextureDimension(D3D11_RESOURCE_DIMENSION_TEXTURE2D),
			LastMapDirection((D3D11_MAP)0), DepthSurface(0), MipLevelLocked(0), NumberOfMipLevels(0),
			ArraySliceLocked(0), NumberOfArraySlices(arraySlices), SampleCount(1), SampleQuality(0),
			HardwareMipMaps(false), ColorFormat(ECF_UNKNOWN)
		{
#ifdef _DEBUG
		...
		//! destructor
		CD3D11TextureArray::~CD3D11TextureArray()
		{
			if (DepthSurface)
			{
		...
		void CD3D11TextureArray::createRenderTarget(const ECOLOR_FORMAT format)
		{
			HRESULT hr = S_OK;
		...
		bool CD3D11TextureArray::createViews()
		{
			if (!Texture)
				return false;
		...
		bool CD3D11TextureArray::createTextureBuffer()
		{
			if (!Texture)
			{
		...
		void* CD3D11TextureArray::lock(bool readOnly, u32 mipmapLevel, u32 arraySlice)
		{
			...
			// Map texture buffer
			int i = D3D11CalcSubresource(MipLevelLocked, ArraySliceLocked, NumberOfMipLevels);
		...
		//! unlock function
		void CD3D11TextureArray::unlock()
		{
			if (!Texture)
				return;
		...
		void CD3D11TextureArray::regenerateMipMapLevels(void* mipmapData)
		{
		...
		bool CD3D11TextureArray::createTexture(u32 flags, IImage **image)
		{
			HRESULT hr = S_OK;
			OriginalSize = image[0]->getDimension();

			bool isCompressed = image[0]->isCompressed();
			bool textureMipmap = image[0]->hasMipMaps();
			...
			{
				if (image[0]->isCompressed() == true)
				{
					switch (image[0]->getColorFormat())
					{
					case ECF_DXT1:
			...
			texDesc.MipLevels = 1;
			texDesc.ArraySize = NumberOfArraySlices;
			...
			else
			{
				...
				initData = new D3D11_SUBRESOURCE_DATA[NumberOfArraySlices * mipCount];
				int id = 0;

				// update number of mipmap
				...
				for (u32 i = 0; i < NumberOfArraySlices; i++)
				{
					...
					if (image[i] != NULL)
					{
						unsigned char *pSrcBits = (unsigned char*)image[i]->lock();

						for (int i = 0; i < mipCount; i++)
						{
							getSurfaceInfo(w, h, format, &numBytes, &rowBytes, nullptr);

							initData[id].pSysMem = (const void*)pSrcBits;
							initData[id].SysMemPitch = static_cast<UINT>(rowBytes);
							initData[id].SysMemSlicePitch = static_cast<UINT>(numBytes);
			...
			if (FAILED(hr))
			{
				os::Printer::log("Could not create array texture", irr::ELL_ERROR);
				return false;
			}
			...
			SMViewDesc.Texture2DArray.MipLevels = texDesc.MipLevels;
			SMViewDesc.Texture2DArray.MostDetailedMip = 0;
			SMViewDesc.Texture2DArray.ArraySize = NumberOfArraySlices;
		...
		ID3D11ShaderResourceView* CD3D11TextureArray::getShaderResourceView() const
		{
			return SRView;
		}

		void CD3D11TextureArray::getSurfaceInfo(int width, int height, DXGI_FORMAT fmt, int* outNumBytes, int* outRowBytes, int* outNumRows)
		{
			int numBytes = 0;
			int rowBytes = 0;
			int numRows = 0;
		...
		int CD3D11TextureArray::bitsPerPixel(DXGI_FORMAT fmt)
		{
			switch (fmt)
			{
