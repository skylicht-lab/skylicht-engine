...
//#endif
	...
	{
#if !defined (_IRR_XBOX_PLATFORM_) && !defined(_IRR_WINDOW_UNIVERSAL_PLATFORM_)
...
namespace os
{
	...
	u32 Timer::getRealTime()
	{
		...
		return (u32) ((systemtime.tv_sec * 1000UL) + (systemtime.tv_nsec / 1000000UL));
#elif defined(_IRR_IOS_PLATFORM_)
        const int64_t kOneMillion = 1000 * 1000;
        static mach_timebase_info_data_t s_timebase_info;
        static bool s_initTimeBase = true;
        
        if (s_initTimeBase == true) {
            (void) mach_timebase_info(&s_timebase_info);
            s_initTimeBase = false;
        }
        // mach_absolute_time() returns billionth of seconds,
        // so divide by one million to get milliseconds
        return (u32)((mach_absolute_time() * s_timebase_info.numer) / (kOneMillion * s_timebase_info.denom));
#else
		timeval tv;
		gettimeofday(&tv, 0);
