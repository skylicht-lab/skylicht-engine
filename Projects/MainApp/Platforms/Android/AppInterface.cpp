...
//#include "UIControl/CUIEditControl.h"
...
extern "C" {
	...
	// get current edittext value
	const char* applicationGetEditFieldValue()
	{
		/*
		CUIEditControl *editControl = CUIEditControl::getCurrentEditControl();

		if (editControl == NULL)
			return NULL;

		return editControl->getValue();
		*/
		return NULL;
	}

	// applicationSetEditFieldValue
	// set current edittext value
	void applicationSetEditFieldValue(const char *value)
	{
		...
		if (editControl != NULL)
		{
			printf("applicationSetEditFieldValue: %s", value);
			editControl->setValue(value);
			editControl->setActiveEdit(false);
		}
		*/
	}
